...
public class AdvancedUI extends JFrame {
	...
	//Advenced初始設定的包含(不同levelUI要調整的部分)：16*16，bomb40
	private int xMax=16;
	private int yMax=16;
	private int bombNum=40;
	private int gameLevel=2;
	private GameControl<AdvancedUI> control;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					AdvancedUI frame = new AdvancedUI();
	...
	public AdvancedUI() {
		...
		layeredPane.setBounds(10, 10, 499, 566);
		contentPane.add(layeredPane);
		
		//1)時間與炸彈個數計算放最下面
		JPanel panel_2 = new JPanel();
		panel_2.setBounds(10, 10, 480, 57);
		...
		control=new GameControl<AdvancedUI>(this,newGame);//遊戲初始化設定
	...
	private void resetGame()
	{
		...
		control=new GameControl<AdvancedUI>(this,resetedGame);//遊戲初始化設定
		//確認該放哪
		GameControl.bomb=bombNum;
		GameControl.flag=0;
		GameControl.buttonLast=xMax*yMax;
	...
	private void gameOver(String type)//win or over(image引數)
	{
		...
		if(type.equals("lose"))
		{
			...
			Timer delayShow2=new Timer(5000,e1->{
				...
				if(respond==JOptionPane.OK_OPTION) new AdvancedUI().setVisible(true);
