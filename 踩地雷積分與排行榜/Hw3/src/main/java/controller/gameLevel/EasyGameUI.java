...
public class EasyGameUI extends JFrame {
	...
	//Easy初始設定的包含(不同levelUI要調整的部分)：9*9，bomb10
	private int xMax=9;
	private int yMax=9;
	private int bombNum=10;
	private int gameLevel=1;
	private GameControl<EasyGameUI> control;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					EasyGameUI frame = new EasyGameUI();
	...
	public EasyGameUI() {
		...
		layeredPane.setBounds(10, 10, 291, 361);
		contentPane.add(layeredPane);
		
		//1)時間與炸彈個數計算放最下面
		JPanel panel_2 = new JPanel();
		panel_2.setBounds(10, 10, 270, 57);
		...
		control=new GameControl<EasyGameUI>(this,newGame);//遊戲初始化設定
	...
	private void resetGame()
	{
		...
		control=new GameControl<EasyGameUI>(this,resetedGame);//遊戲初始化設定
		//確認該放哪
		GameControl.bomb=bombNum;
		GameControl.flag=0;
		GameControl.buttonLast=xMax*yMax;
	...
	private void gameOver(String type)//win or over(image引數)
	{
		...
		if(type.equals("lose"))
		{
			...
			Timer delayShow2=new Timer(5000,e1->{
				...
				if(respond==JOptionPane.OK_OPTION) new EasyGameUI().setVisible(true);
