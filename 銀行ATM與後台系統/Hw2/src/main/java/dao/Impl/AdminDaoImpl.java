...
public class AdminDaoImpl implements AdminDao{
	...
	public void add(Administrator Admin) {
		String sql="insert into administrator(online_user,online_password,access_level) values(?,?,?)";
		try {
			PreparedStatement ps=conn.prepareStatement(sql);
			ps.setString(1, Admin.getOnline_user());
			ps.setString(2, Admin.getOnline_password());
			ps.setInt(3, Admin.getAccess_level());
			ps.executeUpdate();
	...
	public List<Administrator> selectAll() {
		...
		try {
			...
			while(rs.next())
			{
				...
				Admin.setAccess_level(rs.getInt("access_level"));
				listAdmin.add(Admin);
			}
		} catch (SQLException e) {
			System.out.println("wrong SQL");
	...
	public void update(Administrator Admin) {
		String sql="update administrator set online_user=?,online_password=?,access_level=? where id=?";
		try {
			PreparedStatement ps=conn.prepareStatement(sql);
			ps.setString(1, Admin.getOnline_user());
			ps.setString(2, Admin.getOnline_password());
